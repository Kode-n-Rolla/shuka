...
impl SourceExplorer for BattlechainExplorer {
    fn fetch(&self, request: &FetchRequest) -> Result<RawExplorerResponse, ShukaError> {
        ...
        let response = client.get(BASE_API_URL)
            ...
            .send()
            .map_err(|err| ShukaError::Explorer(format!("failed to send request to Battlechain: {err}")))?;

        let status = response.status();
        if !status.is_success() {
            return Err(ShukaError::Explorer(format!("Battlechain API returned unsuccessful status: {status}")));
        }
